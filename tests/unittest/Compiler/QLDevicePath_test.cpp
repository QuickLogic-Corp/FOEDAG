...
TEST(GeneratedDevicePathHelpers, PathIsInsideTreatsAnEmptyAncestorAsContainingNothing) {
  EXPECT_FALSE(pathIsInside("/a/b/c", std::filesystem::path()));
  EXPECT_TRUE(pathIsInside("/a/b/c", "/a/b"));
  EXPECT_TRUE(pathIsInside("/a/b", "/a/b"));
  EXPECT_FALSE(pathIsInside("/a/bc", "/a/b"));
}
