...
| REQ-003 | When the formula fails and `resources.json` is simply absent, preserve current behavior exactly: report only the formula's derive error via `reportDeviceDataError()`/`ErrorMessage()` and return an empty vector. | M | Same error text and empty result as today for a package with no `resources.json` at all. |
| REQ-005 | When the formula fails and `resources.json` exists but is not a usable fallback (invalid JSON, no entry for this layout, entry missing/invalid keys, or an out-of-range value), report *both* the formula's derive error and the fallback failure reason — the file being present and still wrong is worth surfacing, unlike a genuinely absent file. | M | The reported message names both problems; return value stays an empty vector. |
| REQ-004 | A `resources.json` entry that is not a JSON object, or whose `clb`/`bram`/`dsp`/`io` values are not integers or are outside `int` range, is treated as "no usable fallback" (falls through to REQ-005), not a crash or an uncaught exception. | M | Malformed layout entry or unparseable file is handled and reported as fallback failed, not `std::terminate`/exception. |
