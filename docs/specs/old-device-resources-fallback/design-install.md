...
| 3 | `src/Compiler/QLDeviceManager.cpp:4598-4617` (`deviceResourceInformation()`) | Before calling `reportDeviceDataError(derive_error)`, when `resources_vector.empty() && !derive_error.empty()`: build `deviceTypeDirPath(device_target) / "resources.json"`, and if `FileUtils::FileExists()`, parse it (try/catch around `json::parse`) and call `resourceCountsFromResourcesJson()` with `device_target.device_variant_layout.name`. If that returns non-empty, return it directly (skip `reportDeviceDataError`). Otherwise proceed to the existing `reportDeviceDataError(derive_error)` call using the **original formula error**, unchanged. | Must not call `reportDeviceDataError` twice, and must not report the fallback's own error text when the fallback simply isn't present (REQ-003 requires the *formula's* error message, unchanged) — the fallback failure reason is diagnostic-only, not surfaced. Must not read `resources.json` at all when the formula succeeded (NFR-002) — the read only happens inside the `resources_vector.empty() && !derive_error.empty()` branch, which already excludes the "not yet known" case. |
...
| REQ-003 | #3 — falls through to the existing `reportDeviceDataError(derive_error)` call, unmodified. |
| REQ-004 | #2 — type/presence validation and try/catch around JSON parsing. |
