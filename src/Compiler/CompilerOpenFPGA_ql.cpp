...
int status = ExecuteAndMonitorSystemCommand(command);
#else
  const std::string libPath = QDir::cleanPath(QCoreApplication::applicationDirPath() + "/../lib").toStdString();
  ...
  processedScript += "############################################\n";
  processedScript += "# --- Load the OpenFPGA Tcl extension\n";
  processedScript += "puts [load \"" + libPath + "/openfpga_shell.so\"]\n";
  processedScript += "\n";
