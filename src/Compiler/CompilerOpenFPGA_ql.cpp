...
bool CompilerOpenFPGA_ql::Packing() {
  ...
  if(m_autoLayoutGenerationMode || m_customLayoutGenerationMode) {
    ...
    if ( (m_autoLayoutGenerationMode && (status != 0)) ||
      ...
      // generator devices (AUTO/CUSTOM) may not ship a static SB_MAPS.yml (it is
      // generated by add_layout.py), so m_SBMapsFile can be empty - skip the copy then.
      if(!m_SBMapsFile.empty()) {
    ...
    // Skip the substitution to avoid ReplaceAll() looping on an empty 'from'.
    if(!m_SBMapsFile.empty()) {
...
int status = ExecuteAndMonitorSystemCommand(command);
  ...
  processedScript += "############################################\n";
  processedScript += "### SERVICE COMMANDS GENERATED BY AURORA ###\n";
  processedScript += "############################################\n";
  processedScript += "# --- Create OpenFPGA shell\n";
  processedScript += "puts [set openfpga_sh [std::OpenfpgaShell new]]\n";
  processedScript += "\n";
  processedScript += "# --- Set project directory as a current\n";
  processedScript += "puts [cd \"" + ProjManager()->projectPath() + "\"]\n";
  processedScript += "\n";
  processedScript += "############################################\n";
  processedScript += "### SERVICE COMMANDS GENERATED BY AURORA ###\n";
  processedScript += "############################################\n";
...
bool CompilerOpenFPGA_ql::GenerateIOFloorPlanConstraints(bool forceOverwrite) {
  ...
  // The QDC floorplanning constraints are passed to generate_floorplanning.py
  // via --qdc-filepath and parsed there, so the (potentially large) region
  // group string is no longer built/expanded onto the command line here.
  ...
  if(fs::exists(floor_planning_constraint_filepath)) {
    args.push_back("--qdc-filepath");
    args.push_back(floor_planning_constraint_filepath.string());
  }
